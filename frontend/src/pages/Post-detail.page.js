...
class PostDetailPage extends PureComponent {
  ...
  componentDidMount() {
    const { post, categories, match, dispatch, comments } = this.props;

    if (
      comments.length === 0 ||
      !comments.some(c => c.parentId === match.params.postID)
    )
      this.props.dispatch(fetchComments(match.params.postID));

    isEmpty(post) && this.props.dispatch(fetchPost(match.params.postID));
  ...
  onCreateComment = comment => {
    this.props.dispatch(
      createComment({ ...comment, parentId: this.props.post.id })
    );
  ...
  render() {
    const { post, comments, categories, isLoading } = this.props;

    if (!isLoading && isEmpty(post)) return <Redirect to="/" />;
...
export const mapStateToProps = (state, props) => {
  const { posts, isDonePost, errorPost } = state.posts;
  ...
  comments = sortBy ? sort(sortBy, comments) : sort(sortable[0], comments);
  ...
  return {
    ...
    comments: comments.filter(c => c.parentId === props.match.params.postID),
    categories
  };
};
